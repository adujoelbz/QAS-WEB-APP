...
export default function AdminPortal({ session, onLogout }) {
  ...
  return <div className="app-shell admin-shell"><AdminSidebar activeView={view} setActiveView={setView} mobileNav={mobileNav} collapsed={collapsed} setCollapsed={setCollapsed} closeMobile={() => setMobileNav(false)} onLogout={onLogout} /><div className="main-column"><PortalHeader admin title={titles[view]} profile={session} onMenu={() => setMobileNav(true)} /><main className="content">{busy && <div className="loading-bar">Loading admin workspace...</div>}{error && <div className="api-alert">{error}. Check that the backend is running.</div>}{content}</main></div>{toast && <div className="toast"><span className="toast-check"><Icon name="check" size={13} /></span>{toast}</div>}</div>;
}
